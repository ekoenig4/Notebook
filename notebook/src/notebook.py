...
def format_kwargs(kwargs):
    """
    Format all available {} in kwargs with the values in kwargs
    """
    import re
    ...
    class formatter(dict):
        ...
        def __getitem__(self, key):
            return self.__dict__.get(key, "{%s}" % key)

    def format_kwargs_recursively(kwargs, scope=kwargs):
        scope = dict(scope, **kwargs)

        for key, value in kwargs.items():
            if isinstance(value, str):
                if double_brackets.search(value): continue
                kwargs[key] = value.format_map(formatter(scope))
            elif isinstance(value, dict):
                format_kwargs_recursively(value, scope)

    kwargs = kwargs.copy()
    format_kwargs_recursively(kwargs)
    return kwargs
...
class Notebook:
    ...
    def init_parser(parser):
        parser.add_argument('--config', nargs='*', type=str, help='path to config yaml file', default=None)
    ...
    def from_parser(cls, parser=None, args=None, **kwargs):
        ...
        if args.config:
            if isinstance(args.config, str): args.config = [args.config]
            import yaml
    ...
    def print_namespace(self):
        print("---Arguments---")
        import json
        print(json.dumps(self.namespace, indent=4))
